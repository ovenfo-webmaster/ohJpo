...
public class Jpo {
    ...
    */
	...
	private void setSource(Properties defaultProps, String prefix) {
		this.url = defaultProps.getProperty(prefix+".url");
        this.type = defaultProps.getProperty(prefix+".type");
        this.db = defaultProps.getProperty(prefix+".db");
        this.scheme = defaultProps.getProperty(prefix+".scheme");
        if(this.scheme == null){
            this.scheme = "";
        }
        this.username = defaultProps.getProperty(prefix+".username");
        this.password = defaultProps.getProperty(prefix+".password");
	}
	...
	private void readProp(String source, String propertiesFile, String properties_folder) throws Exception {
		...
		if(source != null && source.length()>0) { // Si cargan desde clase un source jpo.{source}. | @JpoClass( source = "dsprod" )
			...
			this.jdbc = defaultProps.getProperty(defaultSource+".url");
            this.driver = defaultProps.getProperty(defaultSource+".driverClassName");
            this.username = defaultProps.getProperty(defaultSource+".username");
            this.password = defaultProps.getProperty(defaultSource+".password");
