	    ...
	    try {
        	...
        	if(resultados.size()==1) {  // 1: [[[1,"valor"],[1,"valor"]]]
        		if(unRegistro && resultados.get(0) != null && resultados.get(0).size()>0) {
        			...
        			if(resultados.get(0) != null && resultados.get(0).size() > 0) {
        				return resultados.get(0); // return [[1,"valor"],[1,"valor"]]
        			} else {
        				salidaFin = unRegistro ? new HashMap<Object, Object>() : resultados.get(0);
        			}
        			//salidaFin = resultados.get(0); 
        		}
